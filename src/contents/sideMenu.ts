...
const styleSidemenu = () => {
  ...
  if (location.href.includes("https://scombz.shibaura-it.ac.jp/lms/course?")) {
    ...
    for (const contentsDetail of contentsDetails) {
      if (
        contentsDetail.parentNode.parentNode &&
        contentsDetail.parentNode.parentNode.classList.contains("block") &&
        contentsDetail.parentNode.parentNode.classList.contains("clearfix")
      ) {
        setTimeout(() => {
          contentsDetail.parentNode.style.height =
            contentsDetail.clientHeight + "px";
          if (contentsDetail.parentNode.previousElementSibling)
            contentsDetail.parentNode.previousElementSibling.style.height =
              contentsDetail.clientHeight + "px";
        }, 300);
      }
    }
