import type { menuItem } from "../types/sidemenu";

export const hideSideMenu = async () => {
  const sidemenu = document.querySelector(
    "body #contentsWrapper #sidemenu",
  ) as HTMLElement;
  const pagemain = document.querySelector(
    "body #contentsWrapper #pageMain",
  ) as HTMLElement;
...
export const getSideMenuList = (): null | menuItem[] => {
  ...
  if (!sidemenu) return;
  const list = sidemenu.querySelectorAll(
    "a.sidemenu-link,br",
  ...
  const menuItemList: menuItem[] = [...list].map((item) => {
    ...
    const match = classes.find(
      (cls) => cls.match(regex) && cls !== "sidemenu-icon",
    );
    return {
